...
const sumbitWaiter = async () => {
  vForm$.value.$validate()
  if (!vForm$.value.$error) {
    if (isCreate.value) {
      try {
        const data = await WaiterService.create(form_data.value)
            ...
            onClick: (e) => {
              ...
              dialog.success({
                ...
                onPositiveClick: async () => {
                  try {
                    const data = await WaiterService.delete(row.id)
                    message.success("Muvaffaqiyatli o'chirildi")
                    const index = waiters.value.findIndex(
                      (val) => val.id == data.id,
                    )
                    waiters.value.splice(index, 1)
                  } catch (error) {
                    message.error(error)
                  }
                },
      ...
      <n-form>
        ...
        <div style="display: flex; justify-content: flex-end">
          <n-button
            ...
            >Close</n-button
          >
          <n-button type="primary" @click="sumbitWaiter">Submit</n-button>
        </div>
      </n-form>
    </n-modal>
  </div>
